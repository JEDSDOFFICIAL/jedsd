...
export async function assignReviewer(
  ...
  try {
    await axios.put(`/api/paper/${paperId}`, { reviewerId, status ,ReviewerStatus:"NOT_RESPONDED"});
...
export async function reviewerAcceptence(
  ...
  try {
    if(ReviewerStatus === "ACCEPT"){
      await axios.put(`/api/paper/${paperId}`, { reviewerStatus:"ACCEPTED",status:"ON_REVIEW" });

      toast.success("Reviewer accept the paper successfully!");
    }
    if(ReviewerStatus === "REJECT"){
      await axios.put(`/api/paper/${paperId}`, { reviewerStatus:"REJECTED",status:"REVIEWER_ALLOCATION" });
