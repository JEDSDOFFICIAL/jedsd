  ...
  return (
    <div className=" mx-auto ">
      ...
      <main className="md:col-span-1 md:col-start-2">
        <h1 className="lg:text-4xl md:text-3xl text-2xl w-full text-center text-shadow-accent font-bold py-7">
          {paper.title}
        </h1>

        {/* Publisher and Actions */}
        <div className="flex items-center space-x-4 text-sm text-gray-600 py-6  border-t-2 border-black ">
          <p className="text-black text-lg font-bold">
            Publisher: <span className="font-medium ">JEDSD</span>
          </p>
        ...
        <section className="mb-8 text-sm text-gray-700">
          <p className="mb-2">
            <span className="font-semibold">Published In:</span> Journal of Embedded and Digital System Design
          </p>
          
          <p className="mb-2">
            <span className="font-semibold">Publisher:</span> JEDSD
          </p>
          <p className="mb-2">
        ...
        <section className="space-y-6">
          ...
          {/* Contributors (updated to match your data structure) */}
          {paper.contributors && paper.contributors.length > 0 && (
            <div>
              <p className="text-muted-foreground text-sm">Contributors</p>
              <ul className="mt-1 list-disc ml-6 space-y-1 text-sm">
                {paper.contributors.map((c, i) => (
                  <li key={i}>
                    ...
                    {/* Add contribution field if it exists in your ContributorData model */}
                  </li>
                ))}
              </ul>
            </div>
          ...
          {paper.reviewer && (
            <div>
              <p className="text-muted-foreground text-sm">Reviewer</p>
              <p>{paper.reviewer.name}</p>
            </div>
          )}

          {/* Point of Contact (updated to match your data structure) */}
          {paper.pointOfContact && (
            <div>
              <p className="text-muted-foreground text-sm">Point of Contact</p>
              <ul className="text-sm mt-1 space-y-1 ml-2">
                <li>
                  <strong>Name:</strong> {paper.pointOfContact.fullName}
                </li>
                <li>
                  <strong>Email:</strong> {paper.pointOfContact.email}
                </li>
                {paper.pointOfContact.contactNumber && (
                  <li>
                    <strong>Phone:</strong> {paper.pointOfContact.contactNumber}
                  </li>
                )}
                {paper.pointOfContact.affiliation && (
                  <li>
          ...
          {paper.rejectionRemark && (
            <div>
              <p className="text-muted-foreground text-sm">Rejection Remark</p>
              <p className="text-red-600 text-sm">{paper.rejectionRemark}</p>
            </div>
          )}
