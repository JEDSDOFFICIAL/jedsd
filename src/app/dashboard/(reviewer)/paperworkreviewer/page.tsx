...
import ReviewDialogCell from "./_paperaccept/paperAcceptancy";
...
export default function ReviewerReqList() {
  ...
  const updateData = React.useCallback(
    async (paperId: string, type: string) => {
      if (type !== "ACCEPTED_FOR_REVIEW" && type !== "REJECTED_FOR_REVIEW") {
        toast.error("Invalid action type");
        return;
      } else if (type === "ACCEPTED_FOR_REVIEW") {
        setLoading(true);
        await reviewerAcceptence(paperId, "ACCEPT_FOR_REVIEW", fetchData);
        setLoading(false);
      } else {
        setLoading(true);
        await reviewerAcceptence(paperId, "REJECT_FOR_REVIEW", fetchData);
        setLoading(false);
      }
    },
    [fetchData]
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          {
            if(row.original.reviewerStatus !== "ACCEPTED_FOR_REVIEW" &&
            row.original.reviewerStatus !== "REJECTED_FOR_REVIEW" && row.original.reviewerStatus !== "ACCEPTED_FOR_PUBLICATION"){
                ...
                <DropdownMenuSeparator />
                  ...
                  <>
                    <DropdownMenuItem
                      ...
                      onClick={() => updateData(paperId, "ACCEPTED_FOR_REVIEW")}
                    >
                    ...
                    <DropdownMenuItem
                      className="cursor-pointer"
                      onClick={() => updateData(paperId, "REJECTED_FOR_REVIEW")}
                    >
            ...
            else if (row.original.reviewerStatus === "ACCEPTED_FOR_REVIEW"||row.original.reviewerStatus === "ACCEPTED_FOR_PUBLICATION") {
return(
  <ReviewDialogCell paperId={paperId} />
)
            }
            else{
              return(
