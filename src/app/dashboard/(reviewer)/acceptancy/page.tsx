...
export default function ReviewerReqList() {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    const papers = await fetchPapers({
      ...
      // pageIndex: pagination.pageIndex, // Removed because it's not in FetchPapersParams
    });

    if (papers && Array.isArray(papers.papers)) {
      const acceptedPapers = papers.papers.filter(
        (paper: ResearchPaper) => paper.reviewerStatus === "ACCEPTED"
      );
      setTotalPages(papers.totalPages);
      setData(acceptedPapers);
    }
    setLoading(false);
  }, [session?.user?.id]);
  ...
  const columns: ColumnDef<ResearchPaper>[] = React.useMemo(
    () => [
      {
        accessorKey: "id",
        header: () => <div className="text-left">Paper ID</div>,
        cell: ({ row }) => (
          <div className="lowercase">
            {(row.getValue("id") as string).length > 10
      ...
      
      {
        ...
        cell: ({ row }) => (
          <Link href={`/paper/${row.getValue("id")}`} className="lowercase hover:underline hover:text-blue-500">
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="lowercase">
            {row.getValue("status") ? row.getValue("status") : "Pending"}
          </div>
        ),
      },
      {
          ...
          return (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {row.getValue("reviewerStatus") === "ACCEPTED" && row.getValue("status") !== "PUBLISH" && (
                  <>
                    <DropdownMenuItem
